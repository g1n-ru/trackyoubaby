...
class InstructionsPage extends Page
{
    ...
    protected function components(): iterable
    {
        ...
        $counterId = Config::get('tracker.metrica.counter_id') ?: 'YOUR_COUNTER_ID';
        ...
        return $this->card(
            ...
            . $this->table(
                ...
                [
                    ...
                    ['<code>subid2</code>, <code>subid3</code>, <code>subid4</code>', $this->badge('опционально', false), 'Дополнительные метки (объявление, группа и т.д.)'],
                    ['<code>landing</code>', $this->badge('опционально', false), 'URL лендинга для редиректа. Если не указан — DEFAULT_LANDING_URL'],
                ]
            )
    ...
    {
        $retentionDays = Config::get('tracker.data_retention_days', 90);
    ...
    private function renderEnvVariables(): string
    {
        $groups = [
            'Яндекс Метрика' => [
                ['YM_COUNTER_ID', 'ID счётчика Яндекс Метрики', '—', 'tracker.metrica.counter_id'],
                ['YM_TOKEN', 'OAuth-токен Яндекс Метрики', '—', 'tracker.metrica.token', true],
                ['YM_API_URL', 'URL API Метрики', 'https://mc.yandex.ru/collect', 'tracker.metrica.api_url'],
            ],
            '<br><br>Rate Limiting' => [
            ...
            '<br><br>Cookie' => [
                ...
                ['TRACKER_COOKIE_DOMAIN', 'Домен cookie', '—', 'tracker.cookie.domain'],
                ['TRACKER_COOKIE_SECURE', 'Secure-флаг', 'false', 'tracker.cookie.secure'],
            ],
            '<br><br>Прочее' => [
                ['RETRY_MAX_ATTEMPTS', 'Макс. попыток отправки в Метрику', '3', 'tracker.retry.max_attempts'],
                ['DEFAULT_LANDING_URL', 'URL лендинга по умолчанию', 'https://example.com', 'tracker.default_landing_url'],
                ['DATA_RETENTION_DAYS', 'Хранение данных (дней)', '90', 'tracker.data_retention_days'],
            ],
        ];
        ...
        $html = '<div style="overflow-x:auto;margin-bottom:16px">'
            ...
            . '<colgroup><col style="width:22%"><col style="width:32%"><col style="width:22%"><col style="width:24%"></colgroup>'
            . '<thead><tr>'
            . '<th style="' . $thStyle . '">Переменная</th>'
            . '<th style="' . $thStyle . '">Описание</th>'
            . '<th style="' . $thStyle . '">По умолчанию</th>'
            . '<th style="' . $thStyle . '">Текущее значение</th>'
            . '</tr></thead><tbody>';

        foreach ($groups as $groupName => $rows) {
            $html .= '<tr><td colspan="4" style="padding:10px 12px;font-weight:600;font-size:12px;text-transform:uppercase;letter-spacing:0.05em;color:var(--color-base-400);border-bottom:1px solid var(--color-base-stroke)">'
                . $groupName . '</td></tr>';
            ...
            foreach ($rows as $row) {
                ...
                $html .= '<tr>'
                    ...
                    . '<td style="' . $tdStyle . '"><code>' . e($row[2]) . '</code></td>'
